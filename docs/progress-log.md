...
### 缺陷 2：本地库口径是**前复权**，不是后复权

用真实数据对齐（600519，2026-09-17..24，全部重叠交易日）：

| 通道 | 与本地库的比值 | 是否恒定 |
|---|---|---|
| 腾讯 `qfq` | **1.000000**（5/5 天） | 恒定 |
| Wind `aftype=1`（后复权） | 8.64691–8.64701 | **恒定**（5 位有效数字） |
| 腾讯 `hfq` | 7.051085 → 7.085580 | **漂移 0.49%** |

- 腾讯 `qfq` 与本地库**逐日完全相等** ⇒ 本地 `public.daily_bar` 是**前复权**。
  这与 R15 记录的"后复权口径的事实标准"**不符**，已在注册表 note 里改正。
- Wind 后复权与本地序列**严格成比例**（比值恒定）⇒ Wind 侧口径自洽，可作主通道。
- 腾讯 `hfq` 与其自身 `qfq` 的比值在 5 个交易日内漂移 0.49%（无除权事件期间比值
  本应恒定）⇒ **腾讯 `hfq` 序列不可信**；兜底源应当改用 `qfq` 或至少标注差异。

**影响面**：笔/中枢等结构判定对价格整体缩放不变，所以口径差异**不影响**已有的
结构结论（包括 native 笔退化的结论）；但**两通道序列绝不可混用**，跨源校验必须先
统一口径。
